...
class GenerateChannelMerchantPaymentReport extends Command
{
    /**
    ...
    //example : php artisan reports:generateChannelMerchantPaymentReport "2017-03-01 16:00" --channels="13,72"
    protected $signature = 'reports:generateChannelMerchantPaymentReport {date?} {--channels= : Channels to include in payment report. If null, select for all channels.} {--exclude_channels= : Channels to exclude in payment report. If null, no channel is excluded.} {emails?}';
    ...
    protected $reportsDirectory = 'reports/payment_report/channel';
    ...
    public function handle()
    {
        ...
        $channels = (!is_null($this->option('channels')) ? explode(',', $this->option('channels')) : array());
        $excludeChannels = (!is_null($this->option('exclude_channels')) ? explode(',', $this->option('exclude_channels')) : array());
        ...
        // get all active merchants selling on selected channels
        $merchants = Merchant::isActive()->whereHas(
            'channels', function ($query) {
                if(!empty($channels))
                    $query->whereIn('channels.id', $channels);
                if(!empty($excludeChannels))
                    $query->whereNotIn('channels.id', $excludeChannels);
            }
        )
        ->with('channels')
        ->get();
        ...
        $files = array();
        $zip = new ZipArchive;
        $zipFileName = (!empty($channels) ? implode('-', $channels).'_' : '').'merchant_payment_report_'.strtoupper($reportdate->format('MY')).'_'.Carbon::now()->format('Ymd').'.zip';
        ...
        foreach($merchants as $merchant)
        {
            ...
            $resultSet = $this->generateReport($merchant->id, $startdate, $enddate, $this->mailer, $duration, $gst, $channels);
        ...
        if($create)
        {
            ...
            if($s3upload){
                ...
                $email_data['subject'] = $duration . ' Channel Merchant Payment Report ('.$reportdate->copy()->startOfMonth()->format('Y-m-d').' - '.$reportdate->copy()->endOfMonth()->format('Y-m-d').')';
        ...
        Log::info('End generating and emailing '.$duration.' Channel Merchant Payment Report ('.$reportdate->format('MY').') at '. Carbon::now());
    }

    protected function generateReport($merchant_id, $startdate, $enddate, Mailer $mailer, $duration = '', $gst, $channels = []) {
        $dataArray = array();
        $summaries = array();
        ...
        ** sale Report
        */
        $sale = $this->saleQuery($merchant_id, $startdate, $enddate, $gst, $channels);
        $sale['recon']['headers'] = [
            'Channel',
            'Date Ordered',
            'Order #',
        ...
        */
        ...
        $returns = $this->returnsQuery($merchant_id, $startdate, $enddate, $gst, $channels);
        ...
        */
        ...
        $pending = $this->pendingQuery($merchant_id, $startdate, $enddate, $gst, $channels);
    ...
    private function saleQuery($merchant_id, $startDateTime, $endDateTime, $gst, $channels = [], $timezone ="+08:00") {
        $response = array();
        ...
        ->select('order_items.*', 'third_party_report.channel_fees', 'third_party_report.id as tp_id', 'orders.channel_id')
        ...
        ->where('third_party_report.payment_date', '>=', $startDateTime)
        ...
        if(!empty($channels))
            $items = $items->whereIn('orders.channel_id', $channels);
        if(!empty($excludeChannels))
            $items = $items->whereNotIn('orders.channel_id', $excludeChannels);
    ...
    private function returnsQuery($merchant_id, $startDateTime, $endDateTime, $gst, $channels = [], $timezone ="+08:00") {
        ...
        if(!empty($channels))
            $returns = $returns->whereIn('orders.channel_id', $channels);
        if(!empty($excludeChannels))
            $returns = $returns->whereNotIn('orders.channel_id', $excludeChannels);
    ...
    private function pendingQuery($merchant_id, $startDateTime, $endDateTime, $gst, $channels = [], $timezone ="+08:00") {
        ...
        if(!empty($channels))
            $pendings = $pendings->whereIn('orders.channel_id', $channels);
        if(!empty($excludeChannels))
            $pendings = $pendings->whereNotIn('orders.channel_id', $excludeChannels);
    ...
    private function paymentQuery($merchant_id, $sales, $returns, $gst, $startdate, $enddate, $timezone ="+08:00") {
        ...
        /* Remove Hubwire Fee from Channel Report (Fabspy). Only to show in online report.
        ...
        */
        ...
        /* Remove Hubwire Fee from Channel Payment Report (Fabspy). Only to show in Online Payment Report.
