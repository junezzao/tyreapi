...
use App\Modules\Reports\Console\Commands\GenerateMerchantSalesInventoryReport;

class PushMerchantSalesInventoryReport extends Command
{
    /**
    ...
    protected $signature = 'reports:pushMerchantSalesInventoryReport {month} {year}';
     ...
     * @var string
     */
    protected $description = 'Command to PUSH the Merchant Sales/Inventory Report to AMAZON S3';
    ...
    public function handle()
    {
        ...
        $a_date = strtotime("$year-$month-01");
        $startdate = Carbon::createFromFormat('Y-m-d',date("Y-m-d", $a_date),'Asia/Kuala_Lumpur');
        $enddate = Carbon::createFromFormat('Y-m-d',date("Y-m-t", $a_date),'Asia/Kuala_Lumpur');
        ...
        foreach($merchants as $merchant)
        {
            ...
            $s3path = $this->reportsDirectory.'/merchant-downloads/'.$merchant->slug.'/'.$enddate->format('Y').'/'.$enddate->format('m').'/'.$file;
        ...
        Log::info('End Pushing'.$duration.' Inventory Report ('.$startdate->copy()->setTimezone('Asia/Kuala_Lumpur')->format('Y-m-d').' - '.$enddate->copy()->setTimezone('Asia/Kuala_Lumpur')->format('Y-m-d').') at '. Carbon::now());
    }
