...
class PullZaloraOrder extends Command
{
	/**
     ...
     * @var string
     */
    protected $signature = 'Zalora:PullOrders
     ...
     * @var string
     */
    protected $description = 'Pull Zalora orders';
	...
	{
		$this->info('Running... Zalora:PullOrders');

        $channel_type = ChannelType::where('name', 'Zalora')->firstOrFail();
        $channel_type_id = $channel_type->id;
        $zaloraController = $channel_type->controller;
        ...
        // Get all Zalora active channels
        ...
        try
        {
            foreach($channels as $channel)
            {
                ...
                $order_proc = new OrderProc($channel->id, new $zaloraController);
